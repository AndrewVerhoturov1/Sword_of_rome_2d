...
def _audit_step(
    ...
    if usage_available:
        if confirmation_status == "confirmed_request_usage":
            findings.append({
                "id": "step_usage_confirmed",
                "level": "ok",
                "message": f"Step {step_index}: usage confirmed as request-level",
            })
        elif confirmation_status == "confirmed_per_step_delta":
            findings.append({
                "id": "step_usage_delta",
                "level": "ok",
                "message": f"Step {step_index}: usage confirmed as per-step delta",
            })
        else:
            findings.append({
        ...
        # Check for fallback in usage source
        if "total_token_usage" in usage_source and "last_token_usage" not in usage_source:
            findings.append({
                "id": "step_usage_fallback_cumulative",
                "level": "warning",
                "message": (
                    f"Step {step_index}: usage source is cumulative ("
                    f"'{usage_source}'), not request-level"
                ),
            })
    else:
...
def _compute_statuses(
    ...
    # Usage confirmation: based on step-level findings
    step_levels: list[str] = []
    for sf in step_findings:
        for f in sf.get("findings", []):
            step_levels.append(f.get("level", "ok"))

    confirmed_count = sum(1 for sf in step_findings if sf.get("usage_available"))
