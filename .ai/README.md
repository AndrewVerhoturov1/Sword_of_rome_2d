...
- `kilo-notebook-v3` — режим для V3 artifact-producing workflow: безопасный импорт артефактов из внешнего V3 artifact package, проверка manifest, путей и хэшей, запись только разрешённых файлов, journaling. Режим canonically разрешён (Phase 0). `.ai/v3/` subsystem создана (Phases 1-3), manual setup guide доступен (Phase 4). `/v3` shortcut активирован (Phase 7), `scripts/v3/*` созданы, базовый import/check/write/journal flow доказан. Отделён от `kilo-notebook` (`/v1-only`), `kilo-recorder` (`/r1-only`) и V2.
...
- Это explicit entry mode для V3 artifact package import. У человека уже есть V3 package source, и он хочет пройти V3 import route через `Kilo Notebook V3`.
- Codex готовит launch package для `Kilo Notebook V3` (не handoff), используя обязательный шаблон `.ai/prompts/create_v3_shortcut_prompt.md`.
- `/v3` не является новым `Kilo mode`, новым `Agent kind`, auto-import кнопкой или shortcut для external request generation.
- Для prompt-only внешнего вопроса используется `/v1`. Для full external launch package используется `/r1`.

#### `/v3` Runtime Binding

При явном shortcut `/v3` Codex **обязан** использовать шаблон `.ai/prompts/create_v3_shortcut_prompt.md`. Launch package, написанный вручную без шаблона, не считается готовым.
