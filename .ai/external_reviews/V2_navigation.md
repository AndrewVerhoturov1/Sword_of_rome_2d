...
| **Local workspace verification** | После `local_workspace_verified` gate | Обновить статус (`local_workspace_verified`), `Local Workspace State`, `Local Restore Verified`. Обязательный checkpoint между `snapshot_pushed` и `prompt_ready`. |
...
| V2 ID | Date | Status | Topic | Base Commit | Snapshot Commit | Compare | Request Location | Prompt Location | Response Location | Ingest Location | Branch State | Cleanup Decision | Local Workspace State | Local Restore Verified | Summary |
|-------|------|--------|-------|-------------|-----------------|---------|------------------|-----------------|-------------------|-----------------|--------------|------------------|-----------------------|------------------------|---------|

### Допустимые значения полей

**Status:** `draft`, `previewed`, `awaiting_human_push_approval`, `snapshot_pushed`, `local_workspace_verified`, `prompt_ready`, `waiting_external_answer`, `raw_response_captured`, `ingested`, `implementation_planned`, `implemented`, `superseded`, `cleaned`, `cleanup_pending`, `kept_by_decision`
