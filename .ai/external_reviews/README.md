...
## Non-destructive V2 snapshot rule (публикация копии, а не перенос владения)

**`review/v2/...` is a publication copy, not ownership transfer.**

V2 snapshot-ветка — это опубликованная копия для внешнего review, а не перенос локальной работы. После подготовки V2 snapshot должны быть выполнены оба условия:

1. Файлы доступны внешнему reviewer-у по commit-pinned GitHub-ссылкам.
2. Исходный локальный workspace всё ещё содержит нужное WIP-состояние для продолжения работы.

Если выполнен только первый пункт, V2-процесс нельзя считать завершённым.
...
Kilo обязан явно указать метод подготовки snapshot. Допустимые значения:
...
| `same-worktree-with-restore` | **fallback** | Snapshot готовится в том же workspace с обязательным patch/backup restore после push. |
| `other` | **только с явным объяснением** | Любой другой метод; требует явного описания и human approval. |

**Preferred:** `separate-worktree` — Kilo создаёт временный git worktree, копирует туда snapshot, коммитит и пушит оттуда. Основной workspace остаётся нетронутым.

**Fallback:** `same-worktree-with-restore` — если worktree недоступен:
1. До переключения веток сохранить patch/backup локальных изменений.
2. Отдельно сохранить список untracked файлов.
3. Создать review branch, закоммитить и запушить snapshot.
4. Вернуться на исходную ветку.
5. Применить patch/backup обратно.
6. Восстановить untracked файлы.
7. Проверить, что локальный workspace совпадает с ожидаемым состоянием.

### Обязательный checkpoint `local_workspace_verified`

Между `snapshot_pushed` и `prompt_ready` вводится обязательный статус/gate `local_workspace_verified`.

Используется именно `local_workspace_verified`, а не `local_workspace_restored`, потому что:
- при `separate-worktree` restore не требуется — workspace не менялся, нужна только verification;
- при `same-worktree-with-restore` нужен и restore, и verification;
- в обоих случаях gate один и тот же: подтверждение, что локальный workspace содержит нужное WIP-состояние.

До прохождения `local_workspace_verified` **запрещено**:
- выдавать prompt (`prompt_ready`);
- возвращаться к исходной задаче после `/v2` interrupt;
- завершать V2 cycle (cleanup или любой другой финальный статус).

Если verification pending или failed, дальнейший V2 lifecycle блокируется.
...
3. **Выполнить `local_workspace_verified` gate** — проверить, что локальный workspace всё ещё содержит нужные WIP-файлы (см. [Non-destructive V2 snapshot rule](#non-destructive-v2-snapshot-rule-публикация-копии-а-не-перенос-владения)). Без этого gate нельзя продвигаться к `prompt_ready`.
4. **Создать instantiated V2 prompt** — заполнить [`v2_prompt_template.md`](templates/v2_prompt_template.md) с реальными commit-pinned ссылками и сохранить в `.ai/external_reviews/prompts/V2-YYYYMMDD-HHMMSS_prompt.md`.
5. **Показать готовый copy-paste prompt человеку** — prompt должен быть готов к немедленной вставке во внешний чат.
6. **Перевести статус** — сначала в `local_workspace_verified` (workspace проверен), затем в `prompt_ready` (prompt создан), затем, после показа prompt человеку, в `waiting_external_answer`.
7. **Не завершать run финальным report до этого шага.** Если prompt не может быть создан или `local_workspace_verified` не пройден — остановиться с blocked-отчётом, объяснив причину.

Цепочка: `push → verify links → finalize request/safety → local_workspace_verified → create prompt → prompt_ready → show human → waiting_external_answer`. Разрыв `push → prompt`, пропуск `local_workspace_verified` или оставленные `TBD` в request/safety являются нарушением протокола.
...
4. После завершения V2-цикла (push → `local_workspace_verified` → prompt → external answer → ingest → implementation) Kilo **возвращается к исходной задаче** через обязательный `restore-to-working-branch` step или завершает её по новому решению только после такого возврата.
5. **Возврат к исходной задаче после `/v2` interrupt разрешён только после `local_workspace_verified`.** Если verification pending или failed, возврат блокируется, и Kilo обязан завершить run blocked-отчётом.
...
| `local_workspace_verified` | Локальный workspace проверен: WIP-файлы сохранены или восстановлены. Gate перед prompt. |
