...
- `kilo-notebook-v3` = отдельный V3 import/check/write/journal mode для artifact-producing workflow. Режим canonically разрешён (Phase 0). `.ai/v3/` subsystem создана (Phases 1-3), manual setup guide доступен (Phase 4). `/v3` shortcut активирован (Phase 7), `scripts/v3/*` созданы, pilot проведён и доказан.
...
### Block Orchestrator Chat (legacy)

> **Legacy/history only.** `Block Orchestrator Chat` — это роль из закрытой `B1/BOS/block-orchestration` системы (`SP-20260530-b1-boss-rollout`). Не используется как активная роль. Активная модель: `Planner -> Orc documentation-driven subproject execution` (см. [`AGENTS.md`](../../AGENTS.md)).
...
### Role separation for block orchestration (legacy)

> **Legacy/history only.** Эта секция описывает роли из закрытой `B1/BOS/block-orchestration` системы (`SP-20260530-b1-boss-rollout`): Strategist PILOT-005, Main Execution Orchestrator Chat, Block Orchestrator Chat, Executor Run, Block Orchestrator Package, fail-fast preflight-check. Не используется как активный контракт. Активный маршрут: `Planner -> Orc documentation-driven subproject execution`. Сохранено для истории.

### Runtime block orchestration operating contract (legacy)

> **Legacy/history only.** Этот контракт (Junior Block Orchestrator execution boundary, Planned Agent Sequence, Planned Human Checkpoints, Internal-subagent resource policy, Senior orchestrator non-interference, Direct canonical dependencies, Default escalation path) относится к закрытой `B1/BOS/block-orchestration` системе. Не используется как активный operating contract. Сохранено для истории.
...
- **`blocked-no-source-of-truth`** — нет локального файла, reference, правила или иного source of truth, необходимого для продолжения.
- **`blocked-needs-human-decision`** — задача требует выбора между вариантами, которые Kilo не имеет права делать сам.
- **`blocked-v2-recommended`** — blocker достигнут на реальном WIP-коде, и внешний senior review через `/v2` может помочь.

### V2 Recommendation Gate

- Если blocker достигнут на реальном WIP-коде, и внешний senior review может помочь, Kilo обязан завершить попытку как `blocked-v2-recommended` и рекомендовать `/v2 preview` как следующий шаг.
...
Если одна и та же ошибка сохраняется после 3 осмысленных попыток, Kilo обязан остановиться и перейти в blocked report. Это максимум, а не обязательная квота: если следующая попытка уже выглядит рискованной или бессмысленной, Kilo обязан остановиться раньше. Осмысленная попытка — это содержательное изменение подхода, стратегии или инструмента, а не косметическая правка или повтор того же действия.

### Blocked Report Contract

При остановке в любом blocked-статусе Kilo обязан предоставить blocked-отчёт с минимальной структурой:

- **Статус**, **Что пытался сделать**, **Что уже изменил**, **Что ожидалось**, **Что произошло**, **Что уже пробовал**, **Почему остановился**, **Риски дальнейших попыток**, **Нужен ли `/v2`**, **Что нужно от пользователя**, **Возможные следующие шаги / Candidate next steps**.

Полный контракт описан в [`AGENTS.md`](../../AGENTS.md). Blocked-отчёт дополняет, а не заменяет стандартные секции Kilo report: `Human Check`, `Баги и сложности`, `Runtime metadata` и verification semantics.
