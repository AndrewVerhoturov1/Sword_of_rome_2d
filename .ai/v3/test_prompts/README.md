...
- **Человек** открывает этот файл по кликабельной ссылке из ответа `Kilo Notebook V3`, затем копирует содержимое и передаёт его в обычный Kilo code run.
- **Обычный Kilo code run** использует prompt из этого файла для выполнения post-import testing.

Важно:

- canonical source — только эта папка;
- staging-копия не считается заменой;
- `Kilo Notebook V3` должен вернуть именно кликабельную markdown-ссылку на конкретный файл, а не заставлять человека искать prompt вручную.
