...
**Phase 1 завершён (2026-05-27):** этот README и вся `.ai/v3/` структура созданы как discoverable foundation layer.

**Phase 2 завершён (2026-05-27):** все 9 контрактов V3 созданы: request, artifact package, manifest, journal, Codex review, revision, storage, scope, acceptance. V3 описан как формальный процесс, а не как набор идей.

**Phase 3 завершён (2026-05-27):** созданы 4 промпта в [`prompts/`](prompts/README.md) и 5 шаблонов в [`templates/`](templates/README.md). Prompt/template count больше не расходится между plan/docs/navigation.

**Phase 4 завершён (2026-05-27):** manual runtime integration выполнена. Создан setup guide [`docs/manual_kilo_notebook_v3_setup.md`](docs/manual_kilo_notebook_v3_setup.md). Bootstrap portable docs синхронизированы. Global rules больше не отстают от факта существования `.ai/v3/` слоя.

Full operational V3 workflow ещё впереди. Сейчас недоступно:

- `/v3` shortcut не активирован;
- operational scripts `scripts/v3/*` не созданы (Phase 7);
- pilot не проведён (Phase 5);
- journaling и Codex review flow не имеют runtime (Phase 7).
...
|-------|------------|------|
| [`contracts/`](contracts/README.md) | Формальные контракты V3: request, artifact package, manifest, journal, review, revision, storage, scope, acceptance | ✅ Phase 2 завершён |
| [`templates/`](templates/README.md) | Шаблоны: V3 request, manifest.yaml, journal, Codex review, revision request | ✅ Phase 3 завершён |
| [`prompts/`](prompts/README.md) | Промпты: create V3 request, kilo-notebook-v3 mode, Codex review, revision request | ✅ Phase 3 завершён |
| [`docs/`](docs/README.md) | Документация: [setup guide](docs/manual_kilo_notebook_v3_setup.md), safety rules, storage policy, pilot notes | Phase 4 (setup guide), остальное — Phase 3+ |

`templates/` и `prompts/` — наполнены в Phase 3 (шаблоны и промпты). `docs/` — содержит setup guide (Phase 4), остальная документация — в следующих фазах.
