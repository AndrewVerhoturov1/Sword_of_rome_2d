...
- **Post-import testing flow hardened (2026-05-29):** notebook обязан сохранить repo-local tester prompt copy по пути `.ai/v3/test_prompts/<V3-ID>_post_import_test_prompt.md` и выдать человеку явную ссылку/путь; prompt теперь содержит `Execution split proposal` (tester-first negotiation); machine-check report пишется в canonical file `.ai/v3/test_reports/<V3-ID>_machine_check_report.md`; Codex читает этот file как главный источник результатов.
