...
- **Post-import testing flow hardened (2026-05-29):** notebook auto-emit жёстко обязателен; prompt теперь содержит `Execution split proposal` (tester-first negotiation); machine-check report пишется в canonical file `.ai/v3/test_reports/<V3-ID>_machine_check_report.md`; Codex читает этот file как главный источник результатов.
...
Kilo Notebook V3
  ...
  -> создаёт journal draft
  -> обновляет V3 lifecycle entry
  -> если есть POST_IMPORT_TEST_PROMPT.md и mode ≠ waived — auto-emit: показывает prompt человеку

Post-import testing
   -> notebook auto-emit показывает POST_IMPORT_TEST_PROMPT.md человеку
...
| [`test_reports/`](test_reports/) | Machine-check reports от обычного Kilo code run после post-import testing | local, не journal |
