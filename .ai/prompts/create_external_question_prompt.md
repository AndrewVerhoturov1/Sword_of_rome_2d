...
| `V1_navigation.md` | Project-specific индекс прошлых `/v1` ответов в consumer repo. Если файл существует, его нужно передавать в каждом `/v1` prompt и просить внешний чат прочитать его первым из project-specific контекста. | Обязательно, если файл существует |
...
- ссылку на `V1_navigation.md` как project-specific индекс прошлых `/v1`, если файл существует;
...
### Обязательный project-specific индекс `/v1`

Если в consumer repo существует `.ai/external_chats/V1_navigation.md`, Codex должен по умолчанию включать его в **каждый** `/v1` prompt.

Что это даёт:

- внешний чат видит уже накопленный `/v1` контекст;
- меньше риск дублировать уже собранные выводы;
- проще согласовывать новый ответ с прошлой planning history.

Что нужно явно написать в prompt:

- сначала прочитай `V1_navigation.md`;
- если из него видны релевантные прошлые `/v1`, можешь открыть их как дополнительные project-specific sources;
- каждую такую открытую ссылку обязательно добавь в `Context Readback`;
- `V1_navigation.md` не заменяет остальные project-specific документы по текущему вопросу.
...
| `{{Project V1 Navigation Link}}` | Raw URL или blob URL на `.ai/external_chats/V1_navigation.md` в consumer repo. Если файл существует, включай всегда. Рядом дай короткое указание: «Сначала прочитай этот индекс прошлых `/v1`. Если оттуда видны релевантные notebook entry, можешь открыть их как дополнительные project-specific sources и добавь каждую открытую ссылку в Context Readback». Если файла нет, напиши: `отсутствует`. |
...
- **`V1_navigation.md`** обязателен по умолчанию для consumer repo, где этот файл существует. Его задача — дать внешний индекс прошлых `/v1`, а не заменить текущие project-specific документы.
...
- V1_navigation.md (project-specific raw URL): fully read
