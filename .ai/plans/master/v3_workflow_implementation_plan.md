...
Версия: 0.3
Статус: Phase 0 завершён, Phase 1 завершён (2026-05-27), Phase 2 pending
...
Нужно зафиксировать отдельными файлами:
...
- `create_v3_request_prompt.md`
- `kilo_notebook_v3_mode_prompt.md`
- `codex_v3_review_prompt.md`
- `v3_revision_request_prompt.md`
- `v3_request_template.md`
- `v3_manifest_template.yaml`
- `v3_journal_template.yaml`
- `v3_codex_review_template.md`

Важно:

- prompt для внешнего чата должен требовать artifact package, а не совет;
- prompt для `Kilo Notebook V3` должен описывать безопасный import flow;
- prompt для Codex review должен проверять journal и реальные файлы, а не верить ZIP на слово.
...
- validator списков mode/role;
- handoff/launch-package templates;
- model-selection prompts;
- bootstrap/setup helpers;
- manual Kilo UI setup instructions.

Отдельно проверить несогласованности текущего tooling:

- Phase 0 уже синхронизировал base mode/role lists в `validate_kilo_contract.py` (`kilo-notebook`, `kilo-notebook-v3`, `Notebook Agent`). Phase 4 должна добавить V3 package-specific validation/staging helpers, а не повторять Phase 0 mode-list work;
- `Notebook Agent` должен быть везде, где validator ожидает допустимые task roles;
- UI mode list и internal mode list должны расширяться синхронно.

Выход фазы:

- новый режим валиден по preflight;
- новый режим можно честно рекомендовать в launch package;
- bootstrap/docs больше не вводят человека в заблуждение.
