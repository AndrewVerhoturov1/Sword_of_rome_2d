...
Версия: 0.10
Статус: Phase 0 завершён, Phase 1 завершён (2026-05-27), Phase 2 завершён (2026-05-27), Phase 3 завершён (2026-05-27), Phase 4 завершён (2026-05-27), Phase 5A завершён (2026-05-28), Phase 5B завершён (2026-05-28), Phase 5C завершён (2026-05-28), Phase 5D import pilot подтверждён повторным чистым docs-only run в правильном repo (2026-05-29, human verdict pending). Post-import testing correction layer внедрён (2026-05-29).
...
## 9. Post-import testing correction layer (внедрён 2026-05-29)

После доказанного import flow в Phase 5D в V3 workflow внедрён лёгкий post-import testing layer.
...
- machine checks via ordinary Kilo code run,
- human checks via manual review,
- Codex combines both before verdict.
```

### 9.2. Ключевые сущности

- `POST_IMPORT_TEST_PROMPT.md` — опциональный root-level control file V3 package. Не project target file. Содержит Machine checks и Human checks.
- `manifest.post_import_testing` block — `mode: required | optional | waived`. Трёхрежимная модель:
  - `required` — prompt обязателен, testing gate активен;
  - `optional` — prompt опционален, если есть — показывается, но acceptance не блокирует;
  - `waived` — testing не требуется (default для чистого docs_only).
- `Kilo Notebook V3` показывает prompt после импорта при `mode = required` или `mode = optional` (если prompt есть), но не выполняет тесты сам.
- Codex review и acceptance policy учитывают testing gate: для `mode = required` нужен machine-check report или testing waiver.
...
Correction протянут через следующие файлы:

| Файл | Что добавлено |
|------|---------------|
| `v3_artifact_package_contract.md` | `POST_IMPORT_TEST_PROMPT.md` как root-level control file, не project target |
...
| `v3_request_contract.md` | Post-import testing в request, правила по scope |
| `create_v3_request_prompt.md` | Инструкция требовать `POST_IMPORT_TEST_PROMPT.md` |
| `v3_request_template.md` | Секция `Post-Import Testing` в шаблоне |
| `kilo_notebook_v3_mode_prompt.md` | Post-import test prompt display после импорта |
| `v3_codex_review_contract.md` | Testing gate в review flow, waiver mechanism |
| `v3_acceptance_policy.md` | Testing gate и waiver path в acceptance |
| `v3_codex_review_template.md` | Поля testing_status, machine_check_report_ref |
| `v3/README.md` | Post-import testing в process model |
| `v3/contracts/README.md` | Обновлены описания контрактов |
| `v3/prompts/README.md` | Обновлены описания prompts |
| `v3/templates/README.md` | Обновлены описания templates |

### 9.5. Практический flow для человека

1. Package импортируется через `Kilo Notebook V3`.
2. В конце `Kilo Notebook V3` выводит готовый `POST_IMPORT_TEST_PROMPT.md` (если есть и required).
3. Человек копирует Machine checks в обычный Kilo code run.
4. Обычный Kilo code run делает machine-checks и пишет report.
5. Человек делает manual Human checks.
...
3. После этого выбрать следующий большой трек:
   - `Phase 6`: cleanup/hardening V3 lifecycle, policy for accepted journals, статусы в `V3_navigation.md`, явное закрытие pilot-циклов;
   - или `Phase 7`: scripted support (`scripts/v3/*`, validator/staging helpers, import tooling), если сейчас важнее автоматизация, а не только manual flow.
4. Не возвращаться к transport/root-cause investigation как к главному blocker, если workspace root уже совпадает с repo root и новые циклы продолжают идти чисто.
