...
Fix:
- not fixed yet;
- next correction slice must make audit verify monitor claims against upstream evidence, not only against the already-built detail object;
- if that evidence is missing or incomplete, audit must downgrade to `not_verified` / weaker confidence instead of defaulting to strong statuses;
...
- direct local script run over thread `019e9d2a-17d7-7210-ba5e-bd42e6ce6e5f` showed:
  - `audit_status = ok`
  - `usage_confirmation = all_confirmed`
  - `step_attribution_confidence = high`
  - `cost_confidence = per_step_estimated`
- local review concluded these statuses are too strong for the accepted evidence boundary.
