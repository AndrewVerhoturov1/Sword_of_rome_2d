...
- Этот repo про browser-based authoring tool для 2D counter-based варгеймов.
- Sword of Rome-like модуль здесь = первый тестовый модуль, а не весь смысл проекта.
- Публичный project context держим в `canon/`, `references/`, `docs/` и `.ai/`.
- Историю `/v1` сохраняем в `.ai/external_chats/notebook/` и `.ai/external_chats/V1_navigation.md`; это часть публичного контекста.
...
- Допустимые `Kilo mode` в handoff: только `kilo-handoff-runner`, `kilo-debugger`, `kilo-verifier`, `kilo-recorder`, `kilo-notebook`, `kilo-notebook-v3`.
- Допустимые UI-значения в launch package: только `Kilo Handoff Runner`, `Kilo Debugger`, `Kilo Verifier`, `Kilo Recorder`, `Kilo Notebook`, `Kilo Notebook V3`.
...
- `kilo-notebook-v3` предназначен для V3 artifact-producing workflow: безопасный импорт артефактов из внешнего V3 artifact package, проверка manifest, путей и хэшей, запись только разрешённых файлов, journaling. Режим canonically разрешён (Phase 0). `.ai/v3/` subsystem создана (Phases 1-3), manual setup guide доступен (Phase 4). `/v3` shortcut активирован (Phase 7), `scripts/v3/*` созданы, pilot проведён и доказан. `kilo-notebook-v3` отделён от `kilo-notebook` (`/v1-only`), `kilo-recorder` (`/r1-only`) и V2.
...
> **Legacy/history only.** `/b1` — это shortcut из закрытой `B1/BOS/block-orchestration` системы (`SP-20260530-b1-boss-rollout`). Не используется как активный shortcut. Активный маршрут: `Planner -> Orc documentation-driven subproject execution`. Этот shortcut предназначался для planning-only mode, где проектировался `Block Orchestrator Chat` и `Planned Agent Sequence`. Сохранён только для истории.
...
- `/v3` не является:
  ...
  - новым execution tool;
  - auto-import кнопкой;
  - shortcut для external request generation.
...
- Codex готовит launch package для `Kilo Notebook V3` (не handoff), используя обязательный шаблон [`.ai/prompts/create_v3_shortcut_prompt.md`](.ai/prompts/create_v3_shortcut_prompt.md).
...
При явном shortcut `/v3` Codex **обязан** использовать шаблон [`.ai/prompts/create_v3_shortcut_prompt.md`](.ai/prompts/create_v3_shortcut_prompt.md). Launch package, написанный вручную без шаблона, не считается готовым `/v3` launch package и не должен выдаваться пользователю.
...
> **Legacy note:** The old `B1/BOS/block-orchestration` route (Boss Orchestrator, Block Orchestrator Chat, Block Orchestrator Package, Task Control Pack, Junior Orchestrator) is closed and must not be used as the active execution route. Legacy artifacts live under `.ai/subprojects/SP-20260530-b1-boss-rollout/` for history/audit only.

Активная модель выполнения подпроектов:

```text
Planner
  готовит документацию подпроекта
  определяет scope, план, контекст, checkpoints и следующий шаг выполнения
  держит required context маленьким и явным
  не создаёт папки B1-блоков как обычную единицу выполнения

Orc
  это один главный исполнительный чат для подпроекта
  читает текущую документацию подпроекта
  выполняет следующий утверждённый execution slice
  обновляет прогресс/статус через документацию
  вызывает Kilo, V1, V2, V3 или external review только как инструменты

Документация подпроекта
  это внешняя память Planner и Orc
  хранит текущее состояние, план, карту контекста, решения, отчёты о прогрессе и checkpoints
  заменяет старое дерево Boss/B1 execution как активный маршрут
```

Практическое правило:

```text
Один активный подпроект = одна папка документации + один главный маршрут выполнения Orc.
Нет активного дерева папок B1-блоков.
Нет активного слоя Boss над Orc.
Нет слоя Junior Orchestrator как процесса по умолчанию.
```

### Роли в Planner -> Orc

1. **Planner** — готовит и поддерживает маршрут planning/documentation. Не выполняет substantive repo work сам.
2. **Orc** — один главный исполнительный чат. Читает документацию подпроекта, выполняет утверждённый slice, обновляет документацию.
3. **V1 / V2 / V3 / Kilo** — инструменты, а не постоянные слои иерархии. Вызываются Orc по необходимости.
4. **Human** — финальный владелец процесса. Принимает ключевые решения, подтверждает verdict-ы, может остановить процесс.
...
Каждый execution slice и связанные с ним артефакты обязаны содержать `Planned Human Checkpoints`:
...
При scope ambiguity, missing dependency, невозможности выбрать agent path или конфликте инструкций Orc не домысливает и не откладывает самоблокировку. Он сразу возвращает короткий `Blocked / Clarification Request`. Ранняя эскалация — default behavior, а не поздний fallback.

### Legacy: block orchestration (closed)

> **Legacy/history only.** Старая система `B1/BOS/block-orchestration` (Boss Orchestrator, Block Orchestrator Chat, Block Orchestrator Package, Task Control Pack, Junior Orchestrator) закрыта. Артефакты — в `.ai/subprojects/SP-20260530-b1-boss-rollout/`. Не использовать как активный маршрут или шаблон.
...
YOLO-режим не означает «делай бесконечно, пока не сломаешь проект». YOLO-режим означает «работай самостоятельно, но обязан остановиться на контрольной точке, если дальнейшие действия становятся рискованными или бессмысленными».

Kilo обязан остановиться, если:
- дальнейшие действия становятся рискованными (широкое переписывание, изменение архитектуры, удаление файлов вне scope);
- дальнейшие действия становятся бессмысленными (та же ошибка после 3 осмысленных попыток, отсутствие нужных файлов, отсутствие source of truth);
- достигнут blocker, который нельзя обойти локально.

Остановка в статусе `blocked-no-source-of-truth` / `blocked-needs-human-decision` / `blocked-v2-recommended` считается корректным завершением текущей попытки.

### Blocked statuses

Kilo обязан различать минимум три типа остановки:

- **`blocked-no-source-of-truth`** — нет локального файла, reference, правила или иного source of truth, необходимого для продолжения. Дальнейшие попытки без source of truth бессмысленны.
- **`blocked-needs-human-decision`** — задача требует выбора между вариантами, которые Kilo не имеет права делать сам (scope, architecture, policy, exclusion из local-only).
- **`blocked-v2-recommended`** — blocker достигнут на реальном WIP-коде, и внешний senior review через [`/v2`](.ai/external_reviews/README.md) может помочь. Kilo обязан завершить попытку и явно рекомендовать `/v2 preview` как следующий шаг, а не push.

### V2 Recommendation Gate

Если blocker достигнут на реальном WIP-коде, и внешний senior review может помочь, Kilo обязан завершить попытку как `blocked-v2-recommended` и рекомендовать `/v2 preview` как следующий шаг. Это не означает автоматический push или автоматическое создание review branch без участия человека.

Если blocker вызван отсутствием source of truth, private/local-only артефактов или непроверяемым acceptance criterion до стадии review-worthy WIP, Kilo обязан остановиться, но не обязан рекомендовать V2.

**V2 рекомендуется, когда:**
- реальный WIP-код существует и ревью-пригоден;
- проблема техническая (архитектура, баг, дизайн-решение), а не missing file/docs;
- внешний senior review может дать bounded technical advice.
...
- проблема чисто организационная (нужно решение человека, а не технический совет).

### Attempt budget

Если одна и та же ошибка сохраняется после 3 осмысленных попыток, Kilo обязан остановиться и перейти в blocked report. Это максимум, а не обязательная квота: если следующая попытка уже выглядит рискованной или бессмысленной, Kilo обязан остановиться раньше.

Осмысленная попытка — это содержательное изменение подхода, стратегии или инструмента, а не косметическая правка или повтор того же действия. Хаотичные повторные правки без изменения подхода не засчитываются как осмысленные попытки. Новый цикл попыток допустим только при новой гипотезе или новом evidence.

### Blocked Report Contract

При остановке в любом blocked-статусе Kilo обязан предоставить blocked-отчёт, содержащий:

- **Статус:** один из `blocked-no-source-of-truth`, `blocked-needs-human-decision`, `blocked-v2-recommended`.
- **Что пытался сделать:** краткое описание цели.
- **Что уже изменил:** список изменённых файлов и что в них поменялось.
- **Что ожидалось:** ожидаемый результат.
- **Что произошло:** фактический результат.
- **Что уже пробовал:** список попыток (подходов/стратегий), а не просто «пробовал 3 раза».
- **Почему остановился:** конкретная причина остановки.
- **Риски дальнейших попыток:** что может пойти не так при продолжении.
- **Нужен ли `/v2`:** `да` / `нет` с кратким обоснованием.
- **Что нужно от пользователя:** конкретный вопрос или выбор.
- **Возможные следующие шаги / Candidate next steps:** 2-4 возможных следующих шага.

Blocked Report Contract дополняет, а не заменяет стандартные требования к Kilo-отчёту. Если blocked-отчёт оформляется как Kilo report, в нём всё равно должны быть `Human Check`, `Баги и сложности`, `Runtime metadata` и конкретная verification semantics по правилам repo.

### YOLO не отменяет safety gates

YOLO-режим не отменяет следующие правила:
