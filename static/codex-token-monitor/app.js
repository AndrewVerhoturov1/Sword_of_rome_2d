    ...
    if (hasDetails) {
      ...
      tlItems.forEach(function(it) {
        ...
        if (isPlanUpdate) {
          ...
          // v2.18: add read errors if present
          if (it.read_errors && it.read_errors.length) {
            lines.push('**Ошибки чтения:**');
            it.read_errors.forEach(function(re) {
              lines.push('- `' + re.path + '` — ' + re.error_detail);
            });
            lines.push('');
          }
          lines.push('');
...
function buildTimelineItemDetails(it) {
  ...
  // ── v2.18: Usage raw event link (before patch block) ──
  if (lai && lai.usage_raw_event_index != null) {
    h += '<div style="margin-top:4px;padding:4px 8px;background:#1a1a2e;border:1px solid #333;border-radius:4px;font-size:9px">';
    h += '<div style="font-weight:600;font-size:10px;margin-bottom:2px;color:#7af">Usage raw event</div>';
    h += '<div>Usage raw event: #'+lai.usage_raw_event_index+'</div>';
    h += '<div>Usage source: '+escapeHtml(lai.usage_source||'')+'</div>';
    h += '<div>Usage confirmation: '+escapeHtml(lai.usage_confirmation||'')+'</div>';
    if (lai.confidence) h += '<div>Confidence: '+escapeHtml(lai.confidence)+'</div>';
    h += '</div>';
  } else if (lai) {
    h += '<div style="margin-top:4px;padding:4px 8px;background:#1a1a2e;border:1px solid #333;border-radius:4px;font-size:9px;color:#888">';
    h += '<div>Usage raw event: missing</div>';
    h += '<div>Usage source: missing</div>';
    h += '</div>';
  }

  // ── v2.10: apply_patch data (PRIMARY for patch actions — shown before contribution table) ──
  var apd = it.apply_patch_data;
