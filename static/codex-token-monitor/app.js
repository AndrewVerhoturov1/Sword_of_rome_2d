    ...
    if (hasDetails) {
      ...
      tlItems.forEach(function(it) {
        ...
        if (details.available) {
          lines.push('**Пояснение:** ' + details.summary_ru);
          lines.push('');
        }
        lines.push('</details>');
        lines.push('');
...
function buildTimelineItemDetails(it) {
  ...
  // ── Context contribution table (files + non-read commands only) ──
  ...
  if (allObjects.length > 0) {
    h += '<div style="font-weight:600;font-size:10px;margin-bottom:2px">Вклад файлов и команд в контекст этапа</div>';
    h += '<table style="width:100%;border-collapse:collapse;font-size:10px"><thead><tr style="background:#333">';
    h += '<th style="text-align:left;padding:2px 4px">Объект</th><th style="text-align:left;padding:2px 4px">Тип</th><th style="text-align:right;padding:2px 4px">Размер вывода</th><th style="text-align:right;padding:2px 4px">~Токенов</th><th style="text-align:right;padding:2px 4px">Доля</th><th style="text-align:center;padding:2px 4px">Copy</th>';
    h += '</tr></thead><tbody>';
    allObjects.forEach(function(ao) {
      ...
      // v2.10: show read_count and ranges for file reads
